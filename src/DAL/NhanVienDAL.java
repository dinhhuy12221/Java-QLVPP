...
//    }
	...
	public static boolean themNhanVien(NhanVien NhanVien) {
    	try {
			...
			ps.setBytes(8, NhanVien.getAnhNhanVien());
	...
	public static ArrayList<NhanVien> layDanhSachNhanVien(){
		...
		try{
			getConnection();
			String s = "SELECT MA_NV,HO_TEN,DIA_CHI,SDT,NGAY_SINH,A.MA_CHUC_VU,TEN_CHUC_VU,LUONG,ANH_NHAN_VIEN "+
			...
			while(resultSet.next()){
				...
				double luong = resultSet.getDouble(8);
				byte[] anhnhanvien = resultSet.getBytes(9);
				ChucVu chucVu = new ChucVu();
	...
	public static boolean SuaNhanVien(NhanVien nhanvien) {
		try {
			getConnection();
			String s = "UPDATE NHAN_VIEN SET HO_TEN = '"+ nhanvien.getHoTen() +"', DIA_CHI = '" + nhanvien.getDiaChi() + "', SDT = '" + nhanvien.getSoDienThoai() + 
					"', NGAY_SINH = '" + nhanvien.getNgaySinh() + "', MA_CHUC_VU = '" + nhanvien.getChucVu().getMaChucVu() + "', LUONG = '" + nhanvien.getLuong() +"', ANH_NHAN_VIEN = (SELECT  BULKCOLUMN FROM OPENROWSET(BULK  '" + nhanvien.getAnhFilePath() +"', SINGLE_BLOB) AS x)  WHERE MA_NV = '" + nhanvien.getMa() + "'";
			statement = conn.createStatement();
			if(statement.executeUpdate(s) > 0) {
				closeConnection();
				return true;
